...
class EmbeddingsManager {
    ...
    // Immediate save — cancels any pending scheduleSave so it can't overwrite us.
    async save() {
    ...
    // Debounced save — coalesces rapid per-file updates into one write.
    scheduleSave() {
        clearTimeout(this._saveTimer);
...
class SemanticBacklinksPlugin extends obsidian.Plugin {
    async onload() {
        ...
        this.registerEvent(this.app.vault.on('modify', (file) => {
            ...
            // When Remotely Save (or similar) is syncing, many files change at once.
            // Detect that via its public isSyncing flag or by queue depth, then collapse
            // all pending per-file timers into a single bulk re-index after the flood ends.
            const rsSyncing = !!this.app.plugins?.plugins?.['remotely-save']?.isSyncing;
            if (rsSyncing || this.modifyTimers.size >= 8) {
                for (const t of this.modifyTimers.values()) clearTimeout(t);
                this.modifyTimers.clear();
                clearTimeout(this._bulkReindexTimer);
                this._bulkReindexTimer = setTimeout(async () => {
                    this._bulkReindexTimer = null;
            ...
            this.modifyTimers.set(file.path, setTimeout(async () => {
                ...
                if (await this.embeddings.indexFile(file))
                    this.embeddings.scheduleSave();
    ...
    onunload() {
        ...
        for (const t of this.modifyTimers.values()) clearTimeout(t);
        clearTimeout(this._bulkReindexTimer);
        clearTimeout(this.embeddings._saveTimer);
    }
}
